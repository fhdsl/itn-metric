...
```{r}
ga_metrics %>% janitor::clean_names() %>% 
  dplyr::select(website, screen_page_views_per_user, average_session_duration, event_count_per_user, engagement_rate) %>% 
  tidyr::pivot_longer(!website, 
  ...
  dplyr::mutate(website = dplyr::case_when(website == "Advanced Reproducibility in Cancer Informatics" ~ "Advanced Reproducibility", 
                                           TRUE ~ website)) %>% 
  ggplot2::ggplot(ggplot2::aes(y = value, x = website, fill = metric_name)) +
...
```{r}
course_only <- itcr_course_data %>% 
  dplyr::filter(website != "ITN Website") %>% 
  dplyr::filter(website != "OTTR website")

totals <- data.frame(
  learners = c(
    sum(course_only$activeUsers, na.rm = TRUE),
    sum(course_only$total_coursera_enrollments, na.rm = TRUE),
    sum(course_only$total_leanpub_enrollees, na.rm = TRUE)),
