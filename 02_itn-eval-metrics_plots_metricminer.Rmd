...
Summing it up: 
```{r}
apply(github_metrics[-c(1, 6)], 2, sum, na.rm = TRUE)
```
These metrics are only for the last two weeks
```{r}
github_metrics %>% 
  tidyr::pivot_longer(!repo_name, 
                      names_to = "metric", 
                      values_to = "value") %>% 
  dplyr::filter(metric == "unique_views") %>% 
  ggplot2::ggplot(ggplot2::aes(x = reorder(repo_name, -value), y = value)) + 
  ggplot2::geom_bar(stat = "identity") +
  ggplot2::theme_classic() +
  ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust=1)) +
  ggplot2::xlab("") +
  ggplot2::geom_text(ggplot2::aes(label = value), size = 3, vjust = - 1)
...
itcr_course_data %>% janitor::clean_names() %>% 
  ...
  ggplot2::ggplot(ggplot2::aes(x = reorder(website, sort(target_audience, index.return = TRUE)$ix, FUN = identity), y = value, fill = target_audience)) +
  ggplot2::geom_bar(position = "dodge", stat = "identity") +
  ggplot2::theme_minimal() +
  ggplot2::theme(axis.text.x=ggplot2::element_text(angle=60, hjust=1), 
